 ...
 */
export const listFilesInS3 = (bucket: string): Promise<S3.ListObjectsV2Output> => {
  return new Promise((resolve, reject) => {
    if (s3) {
 ...
 */
export const trashFileFromS3 = async (s3Url: string): Promise<void> => {
 ...
 */
export const restoreFileFromS3Trash = (s3Url: string): Promise<void> => {
  const { key } = parseS3Url(s3Url);
  const originalKey = key.replace(/^trash\//, '');
