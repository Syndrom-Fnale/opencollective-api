...
const recordFile = async (
  kind: FileKind,
  url: string,
  { fileName = null, CreatedByUserId = null } = {},
  progressStr = '',
  ...
  const message = `${kind} ${progressStr} recorded for ${url} as ${fileInfo.ContentType} (${
    fileInfo.ContentLength / 1000
  } KB)`;
  if (!DRY_RUN) {
  ...
  if (!options['onlyLongText']) {
    for (let i = 0; i < SIMPLE_FIELDS.length; i++) {
      ...
      const records = await sequelize.query(
        ...
        ${hasDeletedAt ? 'AND "deletedAt" IS NULL' : ''}
        AND NOT EXISTS (
          SELECT 1
      ...
      records.forEach((record, recordIdx) =>
        queue.add(() => {
          const fileAttributes = { CreatedByUserId: record['UserId'] };
    ...
    for (let i = 0; i < LONG_TEXT_FIELDS.length; i++) {
      ...
      const records = await sequelize.query(
        ` WITH all_images AS (
          ...
          ${hasDeletedAt ? 'WHERE "deletedAt" IS NULL' : ''}
        ) SELECT MIN(id), "url", MIN("UserId")
        FROM all_images
        WHERE NOT EXISTS (
          SELECT 1
